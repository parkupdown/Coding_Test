...
function solution(maps) {
  // 재귀를 통해 뿌리 깊게 들어가면서 파악하자
  // 다시 나와야 하니까 방문기록은 reset이 되어야한다.
  let visited = [];
  maps.forEach((item) => {
  ...
  dfs(startIndex[0], startIndex[1], 0, "L");
  const startToLaver = answer;
  answer = 100000000;
  if (startToLaver === 100000000) {
    return -1;
  }
  visited = [];
  maps.forEach((item) => {
  ...
  dfs(laverIndex[0], laverIndex[1], 0, "E");
  const laverToExit = answer;
  if (laverToExit === 100000000) {
    return -1;
  }
